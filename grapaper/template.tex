...
\chapter{绪论}
\section{引言}
近年来，3D重建的技术收到越来越多的关注。经过多年的研究和发展，3D重建已经发展出了不少的方法，
主要包括立体成像、飞行时间法(TOF)和结构光法等。
\section{3D重建的技术综述}
...
\begin{tikzpicture}
  ...
  \foreach \x in {-1,1,...,7} 
    \draw [fill=blue!50!white] (0.5*\x,3.6-0.3*\x) circle (0.2);
  \foreach \x in {0,2,...,6}
    \draw [fill=blue] (0.5*\x,3.6-0.3*\x) circle (0.2);
  \draw [fill=orange](4,0) rectangle (4.2,3);
  \foreach \x in {3,5}
    \draw [fill=green!50!white] (0.5*\x,-0.6+0.3*\x) circle (0.2);
  \foreach \x in {4,6}
    \draw [fill=green] (0.5*\x,-0.6+0.3*\x) circle (0.2);
\end{tikzpicture}
\caption{TOF}
\end{figure}
\item \textbf{结构光} 
...
0 & f_y & c_y\\
0 & 0 & 1
\end{array}\right]$。矩阵$M$包含相机内部参数的描述，故也被称作摄像机的内矩阵\cite{heikkila1997four}。
...
径向畸变分为筒形畸变和枕形畸变，在光学中心的畸变程度为0，随着想感光元件的边缘移动，
畸变程度越显著，参见图~\ref{fig:tongx}。在实际中，这样的畸变比较小，可以利用光学中心($r=0$)位置
周围的泰勒级数展开来描述，通常只取前两项或三项就足够了。假设$(x_{corrected},y_{corrected})$表示由
...
\centering
  \subfloat[]{
    \includegraphics[width=0.42\textwidth]{chessboard_src.eps}
  }
   \subfloat[]{
    \includegraphics[width=0.44\textwidth]{chessboard_src2.eps}
  }
  \caption{(a),棋盘格作为定标板 (b), 定标板上找到的角点}\label{fig:chessboard_src}
\end{figure}

本文的定标板如图~\ref{fig:chessboard_src}(a)所示，一幅图像中共有$7\times7$个角点，即每两个深色方块的接触点。
每个方块的边长为3cm，将左上角的第一个角点设定为世界坐标系的原点，X 轴和Y轴平行于定标板，Z轴垂直于定标板向外。
根据角点的相对关系，很容易确定定标板上每个角点的世界坐标系坐标。图~\ref{fig:chessboard_src}(b)中是从摄像机
拍摄到图像中检测到的角点，其在图像中的位置可以确定成像平面坐标系的坐标，由此建立世界坐标系和成像平面坐标系的
多个点对，并以此计算相机的内外参数。本文中这些参数的计算是调用openCV的cvCalibrateCamera2函数实现的。\\
openCV计算畸变参数的函数为:\\
...
cvCalibrateCamera2函数的输出包括定标点的世界坐标和图像坐标，输出则包含内参数矩阵$M$、畸变系数($k_1,k_2,p_1,p_2$)、
外参数中的旋转矩阵R和平移矩阵T等。函数cvInitUndistortMap和函数cvRemap可用于对径向畸变和切向畸变的校正。内参数矩阵和
外参数矩阵描述了世界坐标系和成像平面参考系之间的转换关系。
\section{投影仪定标}
...
\begin{figure}[!ht]
  \centering
  \begin{tikzpicture}
    \draw[fill=yellow!50!white] (0,0) rectangle (5,5);
    \draw[fill=white] (0.2,0.2) rectangle (0.6,0.6);
    \draw[fill=black] (0.2,0.2) rectangle (0.4,0.4);
    \draw[fill=black] (0.4,0.4) rectangle (0.6,0.6);

    \draw[fill=black] (0.2,4.4) rectangle (0.6,4.8);
    \draw[fill=white] (0.2,4.4) rectangle (0.4,4.6);
    \draw[fill=white] (0.4,4.6) rectangle (0.6,4.8);

    \draw[fill=white] (4.4,4.4) rectangle (4.8,4.8);
    \draw[fill=black] (4.4,4.4) rectangle (4.6,4.6);
    \draw[fill=black] (4.6,4.6) rectangle (4.8,4.8);

    \draw[fill=black] (4.4,0.2) rectangle (4.8,0.6);
    \draw[fill=white] (4.4,0.2) rectangle (4.6,0.4);
    \draw[fill=white] (4.6,0.4) rectangle (4.8,0.6);
  \end{tikzpicture}
  \caption{投影仪校正的定标板}
\end{figure}
...
\begin{figure}[!ht]
  ...
  \caption{投影仪投射定标图像后的定标板}\label{fig:pc}
\end{figure}
