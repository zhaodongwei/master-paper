...
\subsection{结构光技术的发展}
\section{本文章节组织形式}
\chapter{三角模型和摄像机-投影仪系统校正l}
\section{三角模型}
3D重建使用三角测量的方法对物体进行重建，系统中投影仪、被扫描物体和摄像机呈现三角关系，如图~\ref{figure:system}所示。
\begin{figure}[!ht]
...
本系统使用棋盘格作为定标物，调用openCV摄像机校正函数实现摄像机校正。openCV计算畸变参数的函数为:
...
畸变校正的效果见图~\ref{fig:chessboard}。
\begin{figure}[!ht]
\centering
  \subfloat[]{
    \includegraphics[width=0.4\textwidth]{chessboard.eps}
  }
   \subfloat[]{
    \includegraphics[width=0.4\textwidth]{chessboard2.eps}
  }
  \caption{(a),畸变校正前的棋盘格 (b), 畸变校正后的图像及找到的角点}\label{fig:chessboard}
\end{figure}
\subsection{投影仪畸变校正}
