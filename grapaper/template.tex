...
\chapter{绪论}
\section{引言}
3D重建是一个被关注很久，很重要的技术。作为3D信息采集的重要手段，其在很多领域具有广泛的应用，在医学领域，医生可以用CT设备扫描人体内部结构，在文物保护领域，工作人员可以用3D重建进行逆向工程，对珍贵文物建模，在影视领域，3D重建被广泛应用于虚拟角色的生成，在计算机视觉领域，3D 信息的采集是决策和模式识别的重要信息源。

例外一个例子是，3D打印技术越来越人所熟知，医生可以用它打印关节，航天员可以在太空中随时打印需要的部件，据说国内某型号的隐形飞机就使用了3D 打印的零部件。3D 打印的设备变得越来越为人所接受。
而3D模型的建立通常有两种方法，一种是计算机辅助设计（CAD），另一种是对已有物体进行扫描并建立模型。前者的优点是制作精细，缺点是需要事先设计模型各组成部分比例，需要熟练的操作工人，开发周期长，而后者则基于对已有物体三维信息的采集和处理，并在此基础上建立模型，与3D打印形成很好的配合。

上世纪，摩尔定律\cite{schaller1997moore}见证了计算芯片的飞速发展，计算机的计算能力也得到了极大提升，这也为3D信息的采集和处理提供了很好的计算资源保证。
\section{3D重建的技术综述}
...
\begin{enumerate}
  ...
  \item 保持定标板不动，投影仪投射投影仪定标图像，见图~\ref{fig:pro-cam}。
  \item 摄像机拍摄图像(记为$I_p$)，并减去背景$I_c$，提取投影图像的角点，记录角点在所拍摄图像中的位置，如角点未被全部检测出则重新拍摄$I_p$。
  \item 适度改变定标板位置和角度。
  \item 重复步骤2-5共9次，调用cvCalibrateCamera2函数处理步骤2得到的角点坐标对，计算摄像机内外参数，完成摄像机定标
  \item 根据摄像机参数计算投影图像角点的三维信息，建立投影图像角点在世界坐标系下和投影图像下的坐标对，同样调用cvCalibrateCamera2函数计算投影仪内外参数，完成投影仪定标。
\end{enumerate}
\begin{figure}[!ht]
  \centering
    \includegraphics[width=0.8\textwidth]{pro-cam.eps}
  \caption{投影仪投射定标图像后的定标板}\label{fig:pro-cam}
\end{figure}
...
\end{enumerate}
  ...
  \centering
  \subfloat[]{
  \includegraphics[height=0.2\textwidth,totalheight=0.2\textwidth]{gray_6.eps}}
  \subfloat[]{
  \includegraphics[height=0.2\textwidth,totalheight=0.2\textwidth]{gray_ex_channel.eps}}
    \subfloat[]{
  \includegraphics[height=0.2\textwidth,totalheight=0.2\textwidth]{gray_bw.eps}}
