...
3D信息采集的重要途径和有效方法。本文建设了可用的具有高可靠性的3D 重建系统，
...
In the last decade, 3D shape acquisition has received more and more considerable research interests. Since the concept been proposed, people have developed some groups of techniques
to achieve 3D shape acquisition, some important methods include stereo vision, TOF(Time Of Flight) and structured light\cite{sansoni2009state}.
Among those methods, one of the main advantages of structured light
techniques is that it needs less and easier image processing, meanwhile, it can derive high accuracy in the 3D reconstruction result\cite{OptimisedDeBruijn}.
 Based on triangle principle, Structured light techniques take three steps to fulfill final result. The first step is to project some designed patterns onto the surface of the scanned
objects. After that, a camera is controlled to capture the scene and send the pictures to computer for further processes. In the second step, algorithms are applied to establish the correspondence between designed patterns and patterns detected in the captured images. In the final step, 3D coordinates of the object surface are calculated from the correspondence established in the second step within the calibrated triangle model.

In the review of the development history of structured light techniques, the first appeared technique is the time-multiplexing techniques, which is also called multi-shots techniques because multi pattern are projected onto the surface and multi images are taken. Actually there are both advantages and disadvantages using this method. Advantages include high accuracy and resolution and the disadvantage is that its scanning speed is slow. To achieve high resolution and scanning speed at the same time, one-shot structured light system is developed. In one-shot structured light system, there are three vital processes which would effect final reconstruction result, which includes projected patterns, detection of pattern in captured image and the establishment of correspondence between projected and captured patterns. Recently, more and more efforts have been made to generate higher quality result, which makes this method very useful in industry design and cultural heritage protection.

In this paper, we have build a structure light system and this system uses two methods: colored stripe and Gay code as structure light, which belongs to one-shot structured light technique and time-multiplexing technique. To achieve a better reconstruction result, we propose an adaptive color calibration method, and a Discrete Trend Transform (DTT) algorithm to detect patterns. In this system, dark laboratory environments is not needed to derive high-resolution 3D point cloud. 
...
\begin{tikzpicture}
  ...
  \draw [fill=orange](4,0) rectangle (4.2,3);
  \foreach \x in {3,5}
    \draw [fill=gray!50!white] (0.5*\x,-0.6+0.3*\x) circle (0.2);
  \foreach \x in {4,6}
    \draw [fill=gray] (0.5*\x,-0.6+0.3*\x) circle (0.2);
    \draw [->, very thick] (3,1) -- (2,0.4)node[below]{\text{接收调制光}};
\end{tikzpicture}
\caption{TOF}
\end{figure}
  \item \textbf{结构光法}\cite{besl1988active}
\end{enumerate}
...
$M$通道是指将R、G、B三个通道融合为一个通道，避免了分别处理R、G、B三个通道的复杂性，同时结合所投射颜色序列的特殊性， 最大限度地保留了原始线条的
...
在使用DTT的方法定位线条后，还需要确定线条的峰值位置以用于三维点云的计算。峰值定位的方法有多种，一些是精确到
